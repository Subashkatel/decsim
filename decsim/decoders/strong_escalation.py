"""The strong decoder tier on the window side: StrongEscalation plans the
escalation (the strong context window or the forward strong window, the
windows it absorbs, the rephased suffix, and when the deferred strong job
is submitted); a run without the tier uses NoStrongTier. The decoder
side, which destination waits for which strong result, is
