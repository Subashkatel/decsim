...
class StimDevice(seeding._AtomicRunSeedConsumer):
    """Samples Stim circuits and streams raw measurement packets by round.
    ...
    def __init__(
        ...
        self._sampler_by_key: dict = {}
        self._packets_by_key: dict = {}
        self._table_by_key: dict = {}
        self._former_by_key: dict = {}
        self._detection_events_by_key: dict = {}
        self._truth_by_key: dict = {}
    ...
    def sampled_truth(self) -> dict:
        ...
        truth_by_key = {}
        for key, bits in self._truth_by_key.items():
            truth_by_key[key] = _as_int_bits(bits)
        return truth_by_key
    ...
    def logical_observable_truth(
        ...
        sampled = self._truth_by_key.get(operation_id)
        if sampled is None:
            return None
        return _as_int_bits(sampled)

    def sampled_detection_events(self, operation_id) -> tuple[bool, ...]:
        """The shot's detection events, in the circuit's detector order."""
        sampled_events = self._detection_events_by_key.get(operation_id)
    ...
    def begin_operation(
        ...
        if is_stream and operation.stream_offset:
            events = self._detection_events_by_key[key]
            self._detection_events_by_key[operation.id] = events
            self._truth_by_key[operation.id] = self._truth_by_key[key]
            return
        self._sample_shot(key, operation, source_round_count, detector_rounds)

    def form_round(
        self, operation_id, round_index: int, raw_bits
    ) -> tuple[int, ...]:
        """The detection events of one complete round, in detector order.

        Formed at the decoder input from the round's raw packet.
        """
        former = self._former_by_key.get(operation_id)
        if former is None:
            raise KeyError(
    ...
    def finalize_stream_round(
        ...
        packets = self._packets_by_key[key]
        final_packet = packets[table.round_count]
    ...
    def idle_round_payloads(
        ...
        del operation
        binding = self._source_binding_by_key.get(stream_id)
        if stream_id not in self._packets_by_key or binding is None:
    ...
    def _install_run_seed_state(self, prepared_state) -> None:
        (
            self._seed,
            self._sampler_by_key,
            self._packets_by_key,
            self._table_by_key,
            self._former_by_key,
            self._detection_events_by_key,
            self._truth_by_key,
    ...
    def _validated_root_seed(self) -> int:
        """The seed under Stim's public unsigned 64-bit contract."""
        if not isinstance(self._seed, numbers.Integral):
            raise ValueError(
    ...
    def _sampler_for(self, key, circuit):
        sampler = self._sampler_by_key.get(key)
        ...
        self._sampler_by_key[key] = sampler
        return sampler
    ...
    def _sample_shot(
        ...
        self._table_by_key[key] = table
        self._packets_by_key[key] = packets
        self._former_by_key[key] = detector_formation.StreamingDetectorFormer(
            table
        )
        self._detection_events_by_key[key] = formed_events
        self._truth_by_key[key] = formed_truth
        self._detection_events_by_key[operation.id] = formed_events
        self._truth_by_key[operation.id] = formed_truth
    ...
    def _check_finalizer(
        ...
        if key not in self._packets_by_key:
        ...
        if operation.stream_offset + 1 != source_round_count:
...
class RecordedStimDevice(StimDevice):
    ...
    def __init__(self, measurements, shot: int, **settings):
...
def _window_span(window, round_count: int) -> tuple:
    buffer_hi = min(window.buffer_hi, round_count)
    return (window.start_round, window.commit_lo, window.commit_hi, buffer_hi)
