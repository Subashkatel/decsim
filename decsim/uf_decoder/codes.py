...
def planar_code_extract(L):
    ...
    """
    # Tuckett's qecsim (the separate PyPI package, NOT decsim); imported lazily so the toric/
    # repetition builders above need no Tuckett dependency.
...
def rotated_code_extract(L):
    # Tuckett's qecsim (separate PyPI package, NOT decsim); lazy import (toric needs no Tuckett).
