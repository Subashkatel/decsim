...
DecoderInputTransfer --materialized input--> DecoderLocalMemory
...
- `WindowManager` owns window bounds, readiness, dependencies, boundary transformations, and finality. It does not own payload storage, transfer scheduling, or decoder-local allocations.
...
- `DecoderLocalMemory` owns decoder-local allocations and materialized immutable inputs. It is not a second scheduler.

## Baseline

The default is weak-only, one weak decoder, FIFO, unbounded upstream capacity, unbounded decoder-local capacity, and explicit fixed-latency materialization. Strong/replay tokens are absent unless the selected strategy needs them.
