...
Run one:
    ...
    PYTHONPATH=. python -m experiments.run experiments/configs/strong_only.yaml

Results land in `experiments/results/<name>/`: `sweep.csv` (every column),
`sweep.md` (the table), and the figures. Rerunning the same config
...
-> `sweep_report.py` / `plots.py` (rows -> csv, table, figures).
...
- `refactor_lock.py`: the byte-identity lock; its recordings live in
  `results/refactor_lock/` and must stay.
