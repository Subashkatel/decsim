...
Companion to guide/core-design-audit.md (findings with file:line). Behavior
...
|---|---|---|---|---|---|---|
| window_manager.py | 2738 | broken down | strong_tier.py (escalation, ~1280 lines), boundaries.py (~180), logical_ledger.py (~190) | StrategyServicesImpl from decoder_manager (inlined) | 3 dead accessors, 12 shallow forwarders, 4 lifecycle pass-throughs | ~1000 lines: registry and plan; retention (section); arrival; readiness and submission; traffic functions (section); commit and results |
| decoder_manager.py | 958 | broken down | strong_tier.py (strong ledger, cancellation, batching, ~340) ; decoder_memory_transfer.py (input staging, ~80) | | StrategyServicesImpl, 4 shallow forwarders | ~450 lines: pool and queue; dispatch; service; completion; TerminalRecords small class at the bottom |
| controller.py | 521 | broken down | protected_streams.py (~200 incl. stream reservation) ; policies.py (idle relay branches, ~30) | | _patch_for_operation, throwaway fragment 483-484, round_ticks_for | ~200 lines: load, issue, readout relay, idle accounting, instruction relay |
...
| switching.py | 352 | not broken down | validators 243-323 to defaults.py | | paper narration in the class docstring (to guide/) | threshold register, Baseline, Switching hooks |
...
| policies.py | 57 | grows | | idle relay branches from controller | | Ignore, ExtendStream, SeparateDecodeJobs own their relay behavior |
| planner.py, orchestrators.py, rounds.py, devices.py, config.py, codes.py, layouts.py, schedulers.py | small | leave | | | | |
| run_spec.py | 561 | broken down | defaults.py (10 defaults, 5 compatibility rules, factory decode-service check) | capture_primary_result from views.py | 8 post-construction installs, private engine calls | ~350 lines: RunSpec config, _build_once as wiring, results |
| factories.py | 569 | leave | _check_factory_decode_service to defaults.py | | 3 history comments | |
| links.py | 972 | broken down | link_reports.py (JSON reporting 828-972) | | payload_selection, relation types, cross-check 816-819 | ~750 lines: vocabulary, rules, cards, Link, LinkModel.reserve, ledger |
...
| NEW strong_tier.py | | | | window_manager escalation, decoder_manager strong ledger | | ~1650 lines: StrongEscalation, StrongRequestLedger, StrongBatch, SuffixRephase, the registry types |
| NEW boundaries.py | | | | window_manager 2392-2567 and boundary state | | ~200: BoundaryCourier |
| NEW logical_ledger.py | | | | window_manager 2122-2312 | | ~200: LogicalLedger |
| NEW protected_streams.py | | | | controller protected regions and stream reservation | | ~230: ProtectedStreamRegions |
| NEW defaults.py | | | | run_spec defaults and rules, switching validators, factory check | | ~150 |
| NEW link_reports.py | | | | links.py 828-972 | | ~150 |
...
escalation (or None), lifecycle, recovery, strategy, services, submit_fn,
...
### run_spec.py and defaults.py (new)
...
capture_primary_result. defaults.py: default_strategy, default_scheme,
...
### links.py and link_reports.py (new)
