"""The decoder unit's memory: round order, model row layout, capacity."""
    ...
    def __iter__(self):
        ...
        if self.iterations > 1:
            raise AssertionError(
                "one-shot payloads were traversed more than once"
            )
...
def test_module_has_no_stale_helper() -> None:
    """The retired rematerialization helper stays retired."""
    assert not hasattr(decoder_memory, "_rematerialized_fragment")


def test_materialization_orders_rounds_and_preserves_within_round_order() -> (
    None
):
    """Materialization orders rounds without permuting their fragments."""
...
def test_real_window_model_accepts_matching_operation_round_layout() -> None:
    """A real model accepts matching operation, round order and counts."""
    ...
    for detector_id in window_model.detector_ids:
        round_index, position_in_round = window_model.defect_positions[
            detector_id
        ]
...
def test_real_stim_run_reaches_model_backed_materialization_and_completes(
    ...
    settings = MachineSettings(
        workload=WorkloadSettings(
            operations=[operation], rounds_policy=FixedRounds(3)
        ),
    ...
    assert all(
        isinstance(job.dem, WindowErrorModel) for job in model_backed_jobs
    )
...
def test_same_size_divergent_model_row_layout_order_raises() -> None:
    """A same-size round permutation fails instead of moving syndrome bits."""
...
def test_successor_identity_cannot_replace_predecessor_model_rows() -> None:
    """A sorting-first successor cannot occupy predecessor model rows."""
    window_model = WindowErrorModel(
        detector_ids=(10, 20),
        detector_coordinates=None,
        defect_positions={10: (1, 0), 20: (2, 0)},
...
def test_detector_row_length_divergence_raises() -> None:
    """A model and a bit vector of different lengths fail loudly."""
    window_model = WindowErrorModel(
        detector_ids=(10,),
        detector_coordinates=None,
        defect_positions={10: (1, 0)},
...
def test_empty_job_materializes_to_self_identifying_input() -> None:
    """A timing-only job produces an empty frozen input with its identity."""
...
def test_materialized_fragments_remain_safe_aliases_after_upstream_clear() -> (
    ...
    """Frozen fragment aliases outlive the cleared upstream payload list."""
...
def test_direct_values_keep_duplicates_and_mismatched_rounds() -> None:
    """Direct values keep duplicates and mismatched rounds unchecked."""
...
def test_materialized_round_is_frozen_with_field_dependent_hashing() -> None:
    """Materialized rounds are frozen and hash when their fields hash."""
    round_input = MaterializedSyndromeRound(
        operation_id=1,
        round_index=0,
...
def test_materialization_rejects_identity_outside_stable_domain() -> None:
    """Canonical round ordering rejects an unsupported operation identity."""
    with pytest.raises(TypeError):
...
def _unit_job(label, rounds, request_key=None):
    ...
    job = make_job(
        payloads, op_id=1, window_id=len(rounds), request_key=request_key
    )
    job.label = label
    return job


def test_config_is_rounds_per_unit_by_pool_and_absent_pools_are_unbounded() -> (
    None
):
...
def test_window_larger_than_the_unit_memory_stops_the_run() -> None:
    ...
    with pytest.raises(DecoderMemoryCapacityError) as caught:
        memory.deposit(_unit_job("big", (0, 1, 2)))
    assert (
        caught.value.pool,
        caught.value.unit,
        caught.value.requested_rounds,
        caught.value.capacity_rounds,
    ) == ("default", 1, 3, 2)
    assert memory.occupied_rounds == 0
...
def test_run_gives_every_unit_its_own_memory_and_frees_it_at_completion() -> (
    ...
    settings = MachineSettings(
        workload=WorkloadSettings(
            operations=operations, rounds_policy=FixedRounds(3)
        ),
        qpu=QpuSettings(distance=3),
        weak_decoder=DecoderSettings(
            decoder=PerRoundDecoder(tau_us=1.0), units=2
        ),
