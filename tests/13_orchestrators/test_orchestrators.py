...
def test_connected_integration_logs_then_relays_in_decision_order():
    ...
    assert unit.release_waiters(operation(7)) is None
    ...
    events.clear()
    unit.release_waiters(operation(9, requires_return=True))
    assert events[0] == (
        "log", "PauliFrame",
    ...
    events.clear()
    unit.release_waiters(operation(2))
    assert events == []
