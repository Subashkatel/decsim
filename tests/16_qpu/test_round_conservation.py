...
def _expected(cycle, schedule, end_boundary):
    ...
    for patch, ops in schedule.items():
        ...
        for index, (op_id, start, rounds, emits) in enumerate(ops):
            if emits:
                round_ticks += [
                    (start + k * cycle, op_id, k) for k in range(1, rounds + 1)
                ]
            completion = start + rounds * cycle
            next_start = (
                ops[index + 1][1] if index + 1 < len(ops) else end_boundary
            )
...
def _run_schedule(cycle, schedule, rng):
    ...
    for _ in range(rng.randint(0, 5)):
        engine.schedule(
            rng.randint(0, max(finish_tick - 1, 1)), lambda: None, label="noise"
        )
    engine.run()
    end_boundary = (
        finish_tick
        if finish_tick % cycle == 0
...
def test_every_live_cycle_is_accounted_for(seed):
    ...
    for patch in schedule:
        ...
        all_ticks = sorted(
            [tick for tick, _, _ in observed_rounds[patch]]
            + observed_idle[patch]
        )
        silent = {
            start + k * cycle
...
def test_multi_patch_operation_attributes_rounds_to_its_first_patch():
    ...
    assert [(t, patch) for t, _, patch, _ in capture.rounds] == [
        (10, 0),
        (20, 0),
    ]
    assert sorted(capture.idle) == [
        (30, 0),
        (30, 1),
        (40, 0),
        (40, 1),
