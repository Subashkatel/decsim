...
def test_weak_only_pipeline_arithmetic(fabric):
    ...
    assert window.t_first_round == microseconds_to_ticks(
        1 + 2 + 3 + 4
    )  # round 1 published
    assert window.t_data_complete == microseconds_to_ticks(
        6 + 2 + 3 + 4
    )  # round 6 published
    assert window.t_queued == window.t_data_complete
    assert window.t_dispatch == window.t_data_complete
    assert window.t_done == window.t_data_complete + microseconds_to_ticks(
        5 + 10
    )
    (record,) = completed.pauli_frame.snapshot().records
    assert record.tier == "weak"
    assert record.accepted_ticks == window.t_done + microseconds_to_ticks(
        2
    )  # wdo
    assert (
        record.committed_ticks
        == record.accepted_ticks + microseconds_to_ticks(1)
    )  # frame write
...
def test_cwb_delivers_rounds_in_order(fabric):
    ...
    arrival_order = [
        fabric["log_index"](
            completed.observation.log.lines,
...
def test_weak_primary_readiness_is_buffer0_not_sb1(fabric):
    ...
    completed = fabric["switching_run"](
        escalation_probability=0.0, rounds=9, io_trace=True
    )
    ...
    sb1_landing = fabric["log_index"](
        completed.observation.log.lines,
        "received round 6 of op 1 from controller_to_strong_buffer",
    )
...
def test_a_full_buffer_0_stalls_the_controller_instead_of_failing(fabric):
    ...
    settings = MachineSettings(
        workload=WorkloadSettings(
            operations=[fabric["memory_op"](1)], rounds_policy=FixedRounds(12)
        ),
    ...
    recorder = completed.observation.round_events
    assert recorder.packing_drops == 0
    published = [
        (event.round_index, event.tick)
...
def test_upstream_rounds_survive_until_the_input_transfer_lands(fabric):
    ...
    first_release_tick = next(
        tick
        for (tick, occupancy), (_, previous) in zip(timeline[1:], timeline)
...
def test_room_side_rounds_survive_until_the_final_strong_commit(fabric):
    ...
    make_metrics, probes = fabric["occupancy_metrics"]()
    fabric["switching_run"](
        escalation_probability=1.0, rounds=6, make_metrics=make_metrics
    )
    (probe,) = probes
