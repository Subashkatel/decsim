...
A UnitTiming with an initiation interval adds the second number:
results still return after the service latency while a new decode may
START on the same unit every initiation interval, with at most
pipeline_depth in flight. The
...
def _pipelined(inner, initiation_interval_us, pipeline_depth=None):
    """A unit that starts a new decode every initiation interval."""
    timing = UnitTiming(
        (),
        (),
        1.0,
        initiation_interval_us=initiation_interval_us,
        pipeline_depth=pipeline_depth,
    )
    return StagedDecoder(inner, timing)
    ...
    *,
    ...
    settings = MachineSettings(
        workload=WorkloadSettings(
            operations=[
                fabric["memory_op"](op_id) for op_id in range(1, n_ops + 1)
            ],
...
def test_unit_occupancy_equals_service_latency(fabric):
    ...
    windows = _windows(
        _backlog_run(fabric, decoder=PresetLatencyDecoder(SERVICE_US))
    )
    wbd = fabric["DECLARED_US"]["weak_buffer_to_weak_decoder"]

    assert [window.t_data_complete for window in windows] == [
        microseconds_to_ticks(12)
    ] * 4
    assert [window.t_done for window in windows] == [
        microseconds_to_ticks(12 + wbd + SERVICE_US),
        microseconds_to_ticks(12 + wbd + 2 * SERVICE_US),
        microseconds_to_ticks(12 + wbd + 3 * SERVICE_US),
        microseconds_to_ticks(12 + wbd + 4 * SERVICE_US),
    ]
...
def test_input_slot_gives_one_window_of_lookahead_and_no_more(fabric):
    ...
    assert windows[1].t_dispatch == microseconds_to_ticks(
        12
    )  # DMA overlap only
...
def test_pipelined_unit_starts_every_initiation_interval(fabric):
    ...
    decoder = _pipelined(PresetLatencyDecoder(SERVICE_US), INITIATION_US)
    ...
    assert [window.t_dispatch for window in windows] == [
        microseconds_to_ticks(12)
    ] * 6
    assert [window.t_done for window in windows] == [
        landing + microseconds_to_ticks(k * INITIATION_US + SERVICE_US)
        for k in range(6)
    ]
    service_starts = [
        window.t_done - microseconds_to_ticks(SERVICE_US) for window in windows
    ]
    start_gaps = {
        later - earlier
    ...
    assert start_gaps == {
        microseconds_to_ticks(INITIATION_US)
    }  # D: exact cadence
    assert all(start >= landing for start in service_starts)  # F: never early


def test_initiation_interval_remains_a_lower_bound_when_latency_is_short(
    fabric,
):
    ...
    response_us = 1.0
    interval_us = 10.0
    decoder = _pipelined(PresetLatencyDecoder(response_us), interval_us)
    windows = _windows(_backlog_run(fabric, decoder=decoder, n_ops=2), n_ops=2)
    starts = [
        window.t_done - microseconds_to_ticks(response_us) for window in windows
    ]
...
def test_pipeline_depth_bounds_in_flight_work(fabric):
    ...
    # declared capacity.
    decoder = _pipelined(
        PresetLatencyDecoder(SERVICE_US), INITIATION_US, pipeline_depth=2
    )
    windows = _windows(_backlog_run(fabric, decoder=decoder))
    wbd = fabric["DECLARED_US"]["weak_buffer_to_weak_decoder"]

    assert [window.t_done for window in windows] == [
        microseconds_to_ticks(117),
        microseconds_to_ticks(118),
        microseconds_to_ticks(217),
        microseconds_to_ticks(222),
    ]
    assert (
        windows[2].t_done - microseconds_to_ticks(SERVICE_US)
        == windows[0].t_done
    )  # 3rd start = 1st done
    assert windows[3].t_dispatch == windows[0].t_done  # residency freed late
    assert windows[3].t_done == windows[3].t_dispatch + microseconds_to_ticks(
        wbd + SERVICE_US
    )
...
def test_pipelined_escalation_route_refuses(fabric):
    ...
    # it to a pipelined unit refuses loudly instead of silently
    ...
    router = SwitchingRouter(
        weak=weak,
        strong=_pipelined(PresetLatencyDecoder(SERVICE_US), INITIATION_US),
    )
    settings = MachineSettings(
        workload=WorkloadSettings(
            operations=[fabric["memory_op"](1)], rounds_policy=FixedRounds(6)
        ),
        qpu=fabric["declared_qpu"](),
...
def test_pipelined_timing_validates_its_parameters():
    with pytest.raises(ValueError, match="positive"):
        UnitTiming((), (), 1.0, initiation_interval_us=0.0)
    with pytest.raises(ValueError, match="rounds to zero"):
        UnitTiming((), (), 1.0, initiation_interval_us=1e-9)
    with pytest.raises(ValueError, match="at least 1"):
        UnitTiming((), (), 1.0, initiation_interval_us=1.0, pipeline_depth=0)
    with pytest.raises(ValueError, match="needs an initiation_interval_us"):
        UnitTiming((), (), 1.0, pipeline_depth=2)
