...
def test_serial_escalation_timeline_is_exact(fabric):
    ...
    assert fabric["log_tick"](
        lines, "START DECODE mem1 W1"
    ) == microseconds_to_ticks(71.5)
...
def test_parallel_requires_the_csb_margin(fabric):
    ...
    with pytest.raises(
        ...
        fabric["switching_run"](
            escalation_probability=0.0, rounds=9, run_both_at_once=True
        )
...
def test_bulk_strong_batches_queued_escalations_into_one_decode(fabric):
    ...
    router = SwitchingRouter(
        weak=weak, strong=PresetLatencyDecoder(declared["strong"])
    )
...
def test_no_feedback_when_none_is_required(fabric):
    ...
    assert not any(
        row.get("path") in ("frame_to_controller", "controller_to_qpu")
...
def test_decoder_manager_cannot_bypass_the_controller(fabric):
    ...
    completed = fabric["weak_only_run"](rounds=6)
    assert (
        completed.conditional_release.controller is completed.instruction_output
    )
