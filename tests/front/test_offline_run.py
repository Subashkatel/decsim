...
from decsim.front.offline import SweepPointDecoder, decode_shard, merge, plan
...
def test_offline_matches_closed_loop_on_the_weak_tier(tmp_path):
    ...
    offline = [point.decode_shot(seed).logical_failure for seed in range(seeds)]
    assert offline == closed_loop_failures(config, probability, distance, seeds)
...
def test_plan_shard_merge_pipeline(tmp_path, monkeypatch):
    """The array pipeline end to end.

    plan writes the run dir and the shard table, each shard decodes its
    seed range, merge folds the csvs and stamps the manifest finished.
    """
    config_path = write_config(
        tmp_path,
        {
            "sweep": [
                {
                    "physical_error_probability": [0.003, 0.005],
                    "distance": [3],
                    "round_period_us": [1.0],
                    "shots": 5,
                }
    ...
    assert sorted(
        int(row["seed"])
        for row in shots
        if row["physical_error_probability"] == "0.003"
    ) == list(range(5))
    assert [
        (row["distance"], row["physical_error_probability"], row["shots"])
        for row in ler_rows
    ] == [(3, 0.003, 5), (3, 0.005, 5)]
...
def test_offline_refuses_non_sliding_windows(tmp_path):
    ...
    with pytest.raises(ValueError, match="sliding"):
        SweepPointDecoder(config, physical_error_probability=0.001, distance=3)
